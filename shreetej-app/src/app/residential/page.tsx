...
export default async function ResidentialPage() {
  ...
  return (
    <div className="min-h-screen bg-cream text-text-dark font-sans selection:bg-gold/30 selection:text-navy">
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-[8%] max-w-[1400px] mx-auto min-h-[50vh]">
        {residentialProjects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-10">
            {residentialProjects.map(project => (
              <Link href={`/residential/${project.id}`} key={project.id} className="group block bg-white rounded-3xl overflow-hidden shadow-lg border border-transparent hover:border-gold/30 hover:shadow-2xl transition-all duration-300 hover:-translate-y-2">
                ...
                <div className="p-6 sm:p-8">
                  <h3 className="font-serif text-xl sm:text-2xl font-bold text-navy mb-2 group-hover:text-gold transition-colors">{project.title}</h3>
