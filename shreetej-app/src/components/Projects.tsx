...
export default function Projects() {
  ...
  const projects: Project[] = [
    // NEW LAUNCHING
    { id: 999, name: "Shreetej Platinum-5", type: "Mixed", cityArea: "Sangamner", desc: "Residential + Commercial", status: "LAUNCHING SOON", statusColor: "bg-gold", img: "/assets/shreetej/Shreetej-Properties_Display-Images_PDF-1_page-0004-1024x615_4b6e1e87d3.jpg", location: "Sangamner", year: "2026", details: "Exclusive Pre-Launch. Premium apartments & commercial spaces.", amenities: ["Parking", "Lift", "Security"], area: "—" },
  ...
  const residentialAndCommercial = projects.filter(p => p.type !== "Layouts" && p.id !== 999 && (areaFilter === "All" || p.cityArea === areaFilter));
  const layouts = projects.filter(p => p.type === "Layouts" && (areaFilter === "All" || p.cityArea === areaFilter));
  ...
  return (
    <section id="projects" className="py-24 px-[8%] max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col gap-12">
        <ProjectGrid 
          items={residentialAndCommercial} 
          title="Completed Projects" 
          subtitle="Residential & Commercial Developments" 
        ...
        <ProjectGrid 
          items={layouts} 
          title="NA Layouts" 
          subtitle="Clear Title Plot Developments" 
          onSelect={setSelectedProject}
        />
      </div>
...
const ProjectGrid = ({ 
  ...
  <div className="mb-20">
    <div className="flex flex-col items-center mb-12">
      <h3 className="font-serif text-3xl font-bold text-navy mb-2">{title}</h3>
      <div className="h-1 w-20 bg-gold rounded-full mb-4" />
      <p className="text-text-mid text-sm font-medium tracking-wide uppercase">{subtitle}</p>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
      {items.map((project) => (
        <div
          ...
          className="group relative rounded-2xl overflow-hidden cursor-pointer h-[320px] lg:h-[380px] transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl border border-navy/5 shadow-lg"
        >
          <Image
            src={project.img}
            alt={project.name}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 z-10">
            <h4 className="font-serif text-[1.2rem] text-white font-bold mb-1.5 line-clamp-1">
              {project.name}
            </h4>
